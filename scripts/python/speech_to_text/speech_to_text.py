...
class SpeechToText:
    ...
    def __init__(self, models_root_dir='', wav2vec2_model_path='', version='', language_model_path='', split_only=False):
        ...
        if self.split_only==False:
            ...
            #
            if len(version)==0:
                self.version=os.environ["WAV2VEC2_MODEL_VERSION"]
            
            #
            self.processor, self.model, self.vocab, self.ctcdecoder, self.kenlm_ctcdecoder = create(self.wav2vec2_model_path, self.version)
    ...
    def transcribe(self, wav_file_path, max_segment_length=15, max_segment_words=14, withlm=False):
        ...
        for audio_segment in vadSplitter.split_audio_file(wav_file_path):
            ...
            #
            transcription, beam_probability, alignment, timesteps = self.ctc_decode(logits, withlm) 
            ...
            if len(aligned_words) > 0:
                for transcription, seg_start, seg_end, seg_alignment in self.segment(aligned_words, max_segment_length, max_segment_words):                    
                    yield transcription, beam_probability, seg_start, seg_end, seg_alignment
    ...
    def ctc_decode(self, logits, withlm):
        ...
        x = abs(beam_scores[0]).numpy().tolist()
        smx = np.exp(x - np.max(x)) / np.exp(x - np.max(x)).sum()
        smx = smx.round(3)

        beam_probability = smx[0]
        ...
        return beam_string, beam_probability, alignment, int(beam_results.shape[2]) 
